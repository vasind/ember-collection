...
module('manipulate content', function(hooks) {
  ...
  test("adding to the front of the list content", async function(assert) {
    ...
    //content.addObject({name: "Item -123"});

    // this.set('content', A(newContent));
  ...
  test("inserting in the middle of visible content", async function(assert) {
    this.set('content', generateContent(nItems));

    this.setProperties({height, width, itemHeight, itemWidth, content: this.content});
    await render(fixedGridTemplate);

    this.content.insertAt(2, {name: "Item 2'"});
